...
var opt0 = ["省份","市","县区"];//初始值
function _init_area(){  //初始化函数
	for(i=0;i<s.length-1;i++){
	  document.getElementById(s[i]).onchange=new Function("change("+(i+1)+",s)");
	}
	change(0,s);
}

var s2=["l_province","l_city","l_county"];//三个select的name
function _init_place(){  //初始化函数
	for(i=0;i<s2.length-1;i++){
	  document.getElementById(s2[i]).onchange=new Function("change("+(i+1)+",s2)");
	}
	change(0,s2);
}
